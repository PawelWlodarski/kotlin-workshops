...
class KtorAnswers1 {
    ...
    fun `should put number`() {
        val (_, response, result: Result<String, FuelError>) = "http://localhost:${KtorAnswers1Server.serverPort}/addNumber/2"
    ...
    fun `default 404 page should be defined`() {
        repeat(5) {
            ...
            val (_, response: Response, _) = "http://localhost:${KtorAnswers1Server.serverPort}/$random"
                    .httpPost().response()
    ...
    fun `returns BadRequest when number can not be parsed`() {
        repeat(5) {
            ...
            val (_, response: Response, _) = "http://localhost:${KtorAnswers1Server.serverPort}/addNumber/$random"
                    .httpPost().responseString()
    ...
    fun `removes all data`(){
        val (_, response, result) = "http://localhost:${KtorAnswers1Server.serverPort}/all"
    ...
    fun `stores and retrieves numbers`() {
        //given
        "http://localhost:${KtorAnswers1Server.serverPort}/all".httpDelete().response()


        (1 .. 5).map {
            val (_, response: Response, _) = "http://localhost:${KtorAnswers1Server.serverPort}/addNumber/$it"
                    .httpPost().response()

            response.statusCode shouldEqualTo 200
        }

        val (_, response, result) = "http://localhost:${KtorAnswers1Server.serverPort}/numbers"
    ...
    fun `send and receive json`() {
        ...
        val (_, resetResponse,resetResult)="http://localhost:${KtorAnswers1Server.serverPort}/reset"
        ...
        val (_, selectAll, selectAllResult) = "http://localhost:${KtorAnswers1Server.serverPort}/numbers"
                .httpGet().responseString()
