...
fun simpleNews(operation : SimpleNewsDsl.() -> Unit) = SimpleNewsDsl().apply(operation).display()
...
class SingleTag(private val tag:String) : Tag2() {
    override fun matches(other: String): Boolean  = tag == other
}

class MultiTag(private val tags:Set<String>) : Tag2(){
    fun append(other:String) = MultiTag(tags + other)
    override fun matches(other: String): Boolean  = tags.contains(other)
}
...
class MultiNewsDsl {
    ...
    infix fun String.or(other:String) = MultiTag(setOf(this,other))
    infix fun MultiTag.or(other: String) = this.append(other)  //why this can not access to a private field ???
    ...
    class Articles(private val articles: List<Pair<Tag2, String>>){
        fun getFirstMatching(tag:String): String = articles.find { (tag2,_)-> tag2.matches(tag)} ?.second ?: NOT_FOUND
    }
