...
class ExpressionsAndValues : StringSpec() {
    ...
    init {
        "Create money from int".config(enabled = false){    // EXERCISE1
            Money.of(3).price shouldBe 3
            Money.of(null) shouldBe Money.ZERO
        }

        "Int extension method to money".config(enabled = false) { // EXERCISE1
        ...
        "convert nullable string to description".config(enabled = false) { // EXERCISE2
        ...
        "Generate Delivery address in plain text".config(enabled = false) {  // EXERCISE3
        ...
        "summary price".config(enabled = false) { // EXERCISE 4
        ...
        "send email".config(enabled = false) {  // EXERCISE5
            val p1=Product("TV","great tv",300,15)
